  ...
  return (
    <>
      <button 
        ...
        className="absolute right-4 top-1/2 -translate-y-1/2 text-(--color-primary) opacity-80 hover:opacity-100 drop-shadow-[0_0_8px_var(--color-primary)] transition-all p-1 rounded-full"
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-80 bg-(--card-bg) border border-(--border-color) rounded-xl shadow-xl p-4 z-50 animate-in fade-in slide-in-from-top-2">
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-semibold text-(--text-primary)">Smart Input Guide</h3>
            <button title="Close" type="button" onClick={() => setIsOpen(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
              <X size={16} />
            </button>
          </div>
          <div className="space-y-3 text-sm text-gray-600 dark:text-gray-300">
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-2 rounded-md mt-2 text-xs border border-(--border-color)">
